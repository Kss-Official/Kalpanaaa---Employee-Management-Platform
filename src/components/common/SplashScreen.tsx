...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onFinish, autoCloseDelay = 4200 }) => {
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center cursor-pointer select-none overflow-hidden"
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        {/* top-left */}
        <div className="absolute top-0 left-0 w-64 h-64 bg-blue-700/20 rounded-br-full blur-3xl" />
        {/* bottom-right */}
        <div className="absolute bottom-0 right-0 w-64 h-64 bg-indigo-700/20 rounded-tl-full blur-3xl" />
        {/* centre ambient */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(37,99,235,0.14)_0%,transparent_65%)]" />
      </div>
      ...
      {['top-0 left-0','top-0 right-0','bottom-0 left-0','bottom-0 right-0'].map((pos, i) => (
        <div
          ...
          style={{
            borderTop: ['top-0 left-0','top-0 right-0'].includes(pos) ? '1.5px solid rgba(96,165,250,0.45)' : 'none',
            borderBottom: ['bottom-0 left-0','bottom-0 right-0'].includes(pos) ? '1.5px solid rgba(96,165,250,0.45)' : 'none',
            borderLeft: ['top-0 left-0','bottom-0 left-0'].includes(pos) ? '1.5px solid rgba(96,165,250,0.45)' : 'none',
            borderRight: ['top-0 right-0','bottom-0 right-0'].includes(pos) ? '1.5px solid rgba(96,165,250,0.45)' : 'none',
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        ...
        />
        <div
          className="absolute w-[440px] h-[440px] rounded-full border border-blue-500/10"
          style={{
      ...
      <div
        ...
        style={{
          ...
          transition: 'transform 0.85s cubic-bezier(0.34,1.48,0.64,1), opacity 0.85s ease-out',
        }}
      >
        ...
        <div className="relative">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(96,165,250,0.3), transparent 40%, rgba(99,102,241,0.25))',
            }}
          />
          <img
            src={kalpanaLogo}
        ...
        {/* Progress bar */}
        <div className="w-52 sm:w-72 h-[2px] bg-slate-800/80 rounded-full overflow-hidden">
          <div
            className="h-full rounded-full"
            style={{
              background: 'linear-gradient(90deg, #2563eb, #818cf8, #06b6d4)',
              animation: `loadbar ${autoCloseDelay - 500}ms linear forwards`,
            }}
          />
        </div>

        <p className="text-[10px] font-semibold tracking-[0.22em] uppercase text-slate-500 animate-pulse">
