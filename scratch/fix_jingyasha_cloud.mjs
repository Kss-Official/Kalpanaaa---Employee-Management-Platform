...
async function main() {
  ...
  for (const doc of employees) {
    ...
    if (name.includes("jigy") || name.includes("jing") || empId === "kss2407014" || id === "kss2407014" || id === "emp-kss2407014" || id === "8rxh6z3zzutmm26iqta1dhcpa8l1") {
      console.log(`Updating Jingyasha ${id} to sickLeaveBalance = 1, earnLeaveBalance = 1...`);
      await request(`${FIRESTORE_BASE}/employees/${id}?updateMask.fieldPaths=sickLeaveBalance&updateMask.fieldPaths=earnLeaveBalance`, 'PATCH', {
        fields: {
          sickLeaveBalance: { integerValue: "1" },
          earnLeaveBalance: { integerValue: "1" }
        }
      }, token);
