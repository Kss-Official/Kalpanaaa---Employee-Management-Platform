...
async function main() {
  ...
  for (const doc of docs) {
    const name = doc.fields?.fullName?.stringValue || "";
    const joinDate = doc.fields?.joiningDate?.stringValue || doc.fields?.joining_date?.stringValue || "NONE";
    const type = doc.fields?.employmentType?.stringValue || "NONE";
    console.log(`${name} | joinDate: ${joinDate} | employmentType: ${type}`);
  }
}
