/**
 * Set status to 'Terminated' and clear employeeId on duplicate profiles in Firestore
 */
...
async function main() {
  ...
  const targets = ['3ZHI3aTB37StHjR97WEBJQSWT9H3', 'emp-1785997048965'];

  for (const docId of targets) {
    try {
      console.log(`Setting status 'Terminated' on employees/${docId}...`);
      ...
      console.log(`✅ Updated employees/${docId} status to 'Terminated'!`);
    } catch (e) {
      console.error(`❌ Failed employees/${docId}:`, e.message);
    }
  }
}
