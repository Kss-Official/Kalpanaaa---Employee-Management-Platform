...
async function main() {
  console.log("=== CLOUD FIRESTORE AUDIT & VERIFICATION ===");
  const token = await signIn();
  console.log("1. Cloud Auth: Connected OK to", PROJECT_ID);

  const employees = await getAllDocs(token, "employees");
  console.log("2. Employees in Firestore:", employees.length);

  const targets = ["satya", "jason", "mahesh", "ritish", "thabeethal", "asbin", "akash"];
  console.log("\n--- Target Employees Designation & Type in Cloud ---");
  for (const doc of employees) {
    const name = getStr(doc, "fullName");
    const nameLower = name.toLowerCase();
    if (targets.some(t => nameLower.includes(t))) {
      console.log(`- ${getStr(doc, "employeeId")} | ${name} | Desig: [${getStr(doc, "designation")}] | Type: [${getStr(doc, "employmentType")}]`);
    }
  }

  const leaveRequests = await getAllDocs(token, "leaveRequests");
  console.log("\n3. Leave Requests in Cloud:", leaveRequests.length);
  const relevantLeaves = leaveRequests.filter(l => {
    const name = (getStr(l, "employeeName") || "").toLowerCase();
    const id = (getStr(l, "employeeId") || "").toLowerCase();
    return targets.some(t => name.includes(t) || id.includes(t));
  });

  console.log("--- Relevant Leaves in Cloud ---");
  relevantLeaves.forEach(l => {
    console.log(`- ${getStr(l, "employeeName")} (${getStr(l, "employeeId")}) | Type: ${getStr(l, "type") || getStr(l, "leaveCategory")} | Dates: ${getStr(l, "startDate")} -> ${getStr(l, "endDate")} | Status: ${getStr(l, "status")}`);
  });
}
