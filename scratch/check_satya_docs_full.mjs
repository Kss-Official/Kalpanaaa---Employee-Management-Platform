...
async function main() {
  ...
  for (const id of ['emp-1785997525850', 'emp-KSS2407012']) {
    const doc = await request(`${FIRESTORE_BASE}/employees/${id}`, 'GET', null, idToken);
    const keys = Object.keys(doc.fields || {});
    console.log(`\nDoc ${id} fields count:`, keys.length);
    for (const k of keys) {
      if (k === 'faceDescriptor' || k === 'resumeUrl' || k === 'profilePhotoUrl') continue;
